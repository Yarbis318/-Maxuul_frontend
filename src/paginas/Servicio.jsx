...
const Servicio = () => {
  ...
  const { obtenerServicio, servicio, cargando, handleModalReporte, alerta, submitReportesServicio } = useServicios();
  ...
  useEffect(() => {
    socket.on('reporte agragado', reporteNuevo => {
      if (reporteNuevo.servicio === servicio._id) {
        submitReportesServicio(reporteNuevo)
      }
    })
  })
    ...
    <><div className="flex justify-between">
      ...
      <div className="bg-white shadow mt-10 rounded-ls">
        {servicio.reportes?.length ? 
          ...
          <p className='text-center my-5 p-10 '>No hay reportes en este servicio</p>}
      </div>

      {admin && (
        <>
          <div className="flex items-center justify-center justify-between mt-10">
            <p className="font-bold text-xl">Buzos</p>
            <Link
